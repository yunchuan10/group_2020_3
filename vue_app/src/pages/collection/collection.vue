            ...
            <div >
	            ...
	            <v-header v-if="!answer_card_show"  :header_title="''" >
	                ...
	                <div class="header_right header_right_error" slot="right_box">
	               		...
	               		<!--收藏取消-->
	               		<div :class="collection_false?'new_exercise_collect':'new_exercise_unCollect'"  @click ="collection_cancel"></div>
	                </div>
	            </v-header>
            </div>
...
// 单选 判断
import single from '../../components/collections/single'
// 多选
import multiple from '../../components/collections/multiple'
// 填空
import fillBlack from '../../components/collections/fillBlack'
// 问答
import oneAnswer from '../../components/collections/oneAnswer'
...
export default {
    ...
    created() {
        ...
        obj_r.type = this.$route.query.type || 1
		//调用action动作进行数据请求
        this.coll_error_Actions( { data:obj_r, cb: (res)=>{    //回调
                ...
                //显示答题卡时当前题目序号
                this.$store.commit('coll_error_number', { value: this.current_swiper}); //临时保存状态
    ...
    computed: {
        new_exercises_analysis(){
        	return this.$store.state.collection_analysis
        }
    },
    
    methods:{
        ...mapActions(['coll_error_Actions']),
...
//             this.$router.go(0);  // 强制刷新页面
				...
				if(res.statusCode == 200){
					...
					if(this.new_exercises_analysis.Subjects[this.current_swiper-1].SubjectId){
		            	this.new_exercises_analysis.Subjects.forEach((item,index)=>{
		            		if(item.SubjectId == this.new_exercises_analysis.Subjects[this.current_swiper-1].SubjectId && item.SheetId == this.new_exercises_analysis.Subjects[this.current_swiper-1].SheetId){
		...
		swiper_change(index) {
			if(this.new_exercises_analysis.Subjects.length !== 0){
				this.collection_false = this.new_exercises_analysis.Subjects[index-1].selected;
				//试题名称
                this.SheetName = this.new_exercises_analysis.Subjects[this.current_swiper-1].sheetName;
				 //显示答题卡时当前题目序号
                this.$store.commit('coll_error_number', { value: this.current_swiper}); //临时保存状态
