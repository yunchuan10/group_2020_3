    ...
    <div class="swiper-slide_top_multiple">
		<div class="swiperSlideTop_content">
			...
			<ul class="practice_answer" >
	        	...
	        	<!--添加条件判断是否是正确答案的class是的话添加不是的话忽略-->
	            <li class="check_select" @click="setSelectStatus(opt)" :class="(opt.c_type == 'select_red' ? 'select_default' : opt.c_type )"  v-for="(opt,index) in item.Options" :key="index">
	        ...
	        <!--答案显示-->
	        <ul class="answer">
	        	<li class="answer_li ">正确答案：<span v-if="item.answer_str != 'null' && item.answer_str">{{item.answer_str}}</span></li>
	        	<!--  原来显示有我的答案  -->
...
// import { mapState, mapGetters, mapActions } from 'vuex'
    export default {
    	...
    	created(){},
        computed:{
        	// ...mapGetters(['reseve_obj_exercises']),
        },
        methods:{
            setSelectStatus(opt){
